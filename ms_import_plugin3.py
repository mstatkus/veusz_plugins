...
import logging
logging.basicConfig(filename=r'c:\WPy64-3720\vsz.log',level=logging.DEBUG)
logging.info('version 16')
...
class ImportPluginExample(ImportPlugin):
    ...
    def split_to_blocks(self,raw_data,token1='[MS Chromatogram]',token2 = '\n\n'):
        ...
        blocks = re.split(header,raw_data)
        #logging.info(blocks)
        chrom_block_count = 0
        for (i,block) in enumerate(blocks):
            if block == 'MS Chromatogram' :
                data.append(blocks[i+1])
                chrom_block_count +=1
                logging.info('chrom block found '+str(chrom_block_count))
